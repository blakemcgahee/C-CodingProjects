...
int main() {
	...
	try {
		...
		// This block catches any standard exceptions and prints a message.
		std::cerr << "An unexpected error occurred: " << e.what() << std::endl;
	}
	catch (...) {
		// This block catches any other kind of exception.
		std::cerr << "An unknown fatal error occurred." << std::endl;
	}

	// The final pause is placed outside the loop and the try-catch block
	// to ensure it is always reached, preventing the console from closing immediately.
	std::cout << "\n------------------------------------------------\n";
	std::cout << "The program has finished execution.\n";
	std::cout << "Press Enter to exit the program." << std::endl;
	// Clear the buffer of any leftover newline characters from the last input.
	std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
	// Wait for a final keypress from the user.
	std::cin.get();
